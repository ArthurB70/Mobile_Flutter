    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Comida` (`codigoGeladeira` INTEGER, `qrCode` TEXT, `nome` TEXT, `quantidade` INTEGER, `validade` TEXT, PRIMARY KEY (`qrCode`))');
...
class _$ComidaDao extends ComidaDao {
  ...
  @override
  Future<Comida> getComidaByqrCode(String qrCode) async {
    return _queryAdapter.query('SELECT * from Comida where qrCode = ?',
        arguments: <dynamic>[qrCode],
        mapper: (Map<String, dynamic> row) => Comida(
            codigoGeladeira: row['codigoGeladeira'] as int,
            qrCode: row['qrCode'] as String,
            nome: row['nome'] as String,
